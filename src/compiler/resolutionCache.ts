...
export function createResolutionCache(
    ...
    const filesReferencingResolutionMap = new Map<ResolutionWithFailedLookupLocations, Set<Path>>();
    ...
    function clear() {
        ...
        closeTypeRootsWatch();
        sharedCache.clearForCache(cache);
        resolvedModuleNames.clear();
        resolvedTypeReferenceDirectives.clear();
        filesReferencingResolutionMap.clear();
    ...
    function startCachingPerDirectoryResolution() {
        ...
        sharedCache.startCachingPerDirectoryResolution(resolutionHost.getCompilationSettings());
    }
    ...
    function finishCachingPerDirectoryResolution(newProgram: Program | undefined, oldProgram: Program | undefined, skipCacheCompact?: boolean) {
        ...
        if (newProgram !== oldProgram) {
            const releaseFileWatcherOfAffectingLocation = (location: string) => {
                sharedCache.releaseFileWatcherOfAffectingLocation(location, cache);
            ...
            newProgram?.getSourceFiles().forEach(newFile => {
                ...
                for (let i = existing.length; i < expected; i++) {
                    sharedCache.createFileWatcherOfAffectingLocation(newFile.packageJsonLocations![i], /*forResolution*/ false, cache);
                }
        ...
        if (needsGc) {
            ...
            cache.directoryToModuleNameMap.forEach((resolutions, dir, redirectsCacheKey, directoryToModuleNameMap) => {
                resolutions.forEach((resolution, name, mode, key) => {
                    if (filesReferencingResolutionMap.has(resolution)) return;
    ...
    function isInvalidatedResolution(resolution: ResolutionWithFailedLookupLocations | undefined) {
        return !resolution ||
            (considerNonWatchedResolutionAsInvalidated && !filesReferencingResolutionMap.has(resolution)) ||
    ...
    function watchResolution<T extends ResolutionWithFailedLookupLocations, R extends ResolutionWithResolvedFileName>(
        ...
        const filesReferencing = filesReferencingResolutionMap.get(resolution);
        if (filesReferencing) {
            filesReferencing.add(filePath);
            return;
        }
        sharedCache.watchResolution(resolution, cache, getResolutionWithResolvedFileName, redirectedReference);
        filesReferencingResolutionMap.set(resolution, new Set([filePath]));
    ...
    function stopWatchFailedLookupLocationOfResolution(
        ...
        const filesReferencing = filesReferencingResolutionMap.get(resolution)!;
        filesReferencing.delete(filePath);
        if (filesReferencing.size) return;
    ...
    function releaseResolution(resolution: ResolutionWithFailedLookupLocations) {
        const filesReferencing = filesReferencingResolutionMap.get(resolution)!;
        if (filesReferencing.size) return false;
        filesReferencingResolutionMap.delete(resolution);
        if (isResolvedWithGlobalCachePass(resolution)) resolutionsResolvedWithGlobalCache--;
        else if (isResolvedWithoutGlobalCachePass(resolution)) resolutionsResolvedWithoutGlobalCache--;
        sharedCache.releaseResolution(resolution, cache);
        return true;
    }
    ...
    function invalidateResolution(resolution: ResolutionWithFailedLookupLocations) {
        const filesReferencing = filesReferencingResolutionMap.get(resolution)!;
        for (const containingFilePath of filesReferencing) {
    ...
    function createTypeRootsWatch(typeRoot: string): FileWatcher {
        ...
        return resolutionHost.getCompilationSettings().typeRoots ?
            sharedCache.createTypeRootsWatch(typeRoot, cache) :
            noopFileWatcher;
    }
